 ...
 *
 * Schema esperado (columnas nuevas opcionales):
 *   - precio_promocional: si está, muestra el precio original tachado.
 *   - etiqueta: ribbon "Nuevo", "Premium", "Oferta", "Destacado".
 *   - orden: orden de aparición; vacío = 9999 (al final).
 *
 * Edita un valor en el Sheet, refresca el portal y todos los componentes
...
const PLATAFORMAS_FALLBACK: PlataformaCardData[] = [
  { slug: 'enam', nombre: 'ENAM', descripcion: 'Examen Nacional de Medicina. Banco de preguntas con justificaciones y simulacros cronometrados.', precio: 49, duracion_dias: 30 },
  { slug: 'encib', nombre: 'ENCIB', descripcion: 'Examen Nacional de Ciencias Básicas. Más de 1500 preguntas explicadas, organizadas por curso.', precio: 49, duracion_dias: 30 },
  { slug: 'encaps', nombre: 'ENCAPS', descripcion: 'Evaluación Nacional Capacidades Clínicas. Casos clínicos con retroalimentación detallada.', precio: 49, duracion_dias: 30 },
  { slug: 'rm', nombre: 'Residentado Médico', descripcion: 'Preparación integral para el examen de residentado. Cobertura por especialidades.', precio: 79, duracion_dias: 30 },
  { slug: 'essalud', nombre: 'EsSalud', descripcion: 'Plataforma para concursos EsSalud y SERUMS. Material actualizado y simulacros.', precio: 59, duracion_dias: 30 },
  { slug: 'biblioteca', nombre: 'Biblioteca Médica', descripcion: 'Acceso a libros, guías clínicas y material complementario para tu carrera médica.', precio: 39, duracion_dias: 30 },
];
...
function mapApiData(rows: ReadonlyArray<ApiPlataforma>): PlataformaCardData[] {
  return rows.map((p) => ({
    ...
    precio_promocional:
      p.precio_promocional !== undefined && p.precio_promocional !== null
        ? Number(p.precio_promocional)
        : undefined,
    etiqueta: p.etiqueta ? String(p.etiqueta) : undefined,
    orden: p.orden !== undefined ? Number(p.orden) : undefined,
  }));
}
